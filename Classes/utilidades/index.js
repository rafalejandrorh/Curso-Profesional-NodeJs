...
function processTokens(object) {
    let css = ':root {\n';

    for (const category in object) {
        const properties = object[category];
...
async function main() {
    try {
        const data = await fs.readFile('utilidades/tokens.json', 'utf-8');
        const json = JSON.parse(data);

        const css = processTokens(json);
        await fs.writeFile('utilidades/design-tokens.css', css, 'utf-8');
